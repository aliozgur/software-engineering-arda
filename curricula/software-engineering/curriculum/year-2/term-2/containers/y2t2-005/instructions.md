# Docker and Reproducible Development
...
Package applications reproducibly and understand container isolation limits.
...
1. Containerize an API and PostgreSQL-backed development environment.
2. Write a multi-stage Dockerfile where appropriate.
3. Use Compose for local dependencies.
4. Configure environment variables and secrets without committing secrets.
5. Inspect container networking, filesystem layers and volumes.
...
- [ ] Fresh machine needs only documented prerequisites.
- [ ] Images build reproducibly.
- [ ] Persistent database data survives container replacement when intended.
- [ ] Apprentice explains container versus VM.
...
1. What belongs in an image versus runtime configuration?
2. Why is a container not a security boundary by itself?
...
- Ask apprentice to debug a container that cannot reach its database.
