# Savını Ortaya Koyan Görselleştirme
...
Grafik bir cümledir. Kodlamaları, okuyucunun bütün yükü açıklamaya bırakmadan iddiayı çıkarabileceği biçimde tasarlayın.
...
1. Yanıltıcı bir grafiği (3B pasta grafik, kesilmiş eksen veya çift eksen) ve onun dürüst bir karşılığını yeniden oluşturun.
2. Tek bir veri kümesi için dağılım, karşılaştırma ve zaman içindeki değişimi gösteren üç grafik üretin.
3. Her grafik başlığı, görsel öğelerin desteklediği bir iddia olmalıdır.
4. Okuyucunun hâlâ neyi yanlış yorumlayabileceğini yazın.
...
- [ ] Eksenler, birimler ve filtreler etiketlenir.
- [ ] Yanıltıcı örnek yalnızca değiştirilmez, açıklanır.
- [ ] Renk dekorasyon için değil, anlam taşımak için kullanılır.
- [ ] Çırak geometri seçimini (bar, line veya scatter) gerekçelendirebilir.
...
1. En iyi grafiğinizde en çok işi hangi görsel kodlama yaptı?
2. İddiayı daha açık kılmak için neyi çıkarırdınız?
...
- Başlığı kapatın ve yalnızca görsel öğelerden iddiayı ifade etmesini isteyin.
- Sırasız kategorilerde gökkuşağı renk haritalarını reddedin.
